...
const ordersSchema = z.object({
  ...
  paymentIntentId: z.string(), // 🔹 Add this field
...
export const createCheckoutSession = async (
  ...
  while (retryCount < MAX_RETRIES) {
    ...
    try {
      ...
      const result = ordersSchema.safeParse(req.body);
      if (!result.success) {
      ...
      const stripeSession = await stripe.checkout.sessions.create({
        ...
        mode: 'payment',
        success_url: `${process.env.FRONTEND_URL}/shop/complete?order_id=${order[0]._id}`, // Pass order ID
        cancel_url: `/`,
        metadata: {
...
export const verifyPayment = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    // Retrieve payment intent from Stripe using the stored paymentIntentId
